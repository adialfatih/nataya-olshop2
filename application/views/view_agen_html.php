    ...
    <script>
                ...
                .then(data => {
                    ...
                    data.forEach(item => {
                        let row = `<tr id="tr${item.no_sj}">
                            ...
                            <td><span style='padding:5px 10px;font-size:10px;background-color:#f57520;color:white;border-radius:3px;' onclick="openModalHide('${item.no_sj}','${item.lunas}')">Hide</span></td>
                        </tr>`;
        ...
        function openModalHide(sj,id){
            if(id == "Lunas"){
                Swal.fire({
                title: "Sembunyikan ?",
                text: "Anda tidak akan bisa melihat ini lagi.",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url:"<?=base_url();?>reseller2/sembunyikansj",
                        type: "POST",
                        data: {"sj" : sj},
                        cache: false,
                        success: function(dataResult){
